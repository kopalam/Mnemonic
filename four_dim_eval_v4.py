#!/usr/bin/env python3
"""Mnemonic 四维评估脚本 v4 - 真实session数据 + Keyword优先策略"""
...
REAL_NS = "hermes:boss:hnoe:session_import"
...
class FourDimEvaluator:
    ...
    async def test_d4_relevance(self):
        """D4: 搜索相关性 - 真实session数据 + Keyword优先策略"""
        print("\n" + "="*60)
        ...
        test_queries = [
            # 项目信息
            {"q": "zSunKoin交易周期", "a": "15分钟", "category": "project"},
            {"q": "zSunKoin杠杆", "a": "3-10x", "category": "project"},
            {"q": "zSunKoin核心策略", "a": "趋势跟随", "category": "project"},
            {"q": "zSunKoin用什么数据库", "a": "TimescaleDB", "category": "project"},
            {"q": "zSunKoin数据采集", "a": "CCXT", "category": "project"},
            {"q": "Mnemonic API端口", "a": "8010", "category": "project"},
            {"q": "Mnemonic用什么框架", "a": "FastAPI", "category": "project"},
            {"q": "Mnemonic数据库", "a": "pgvector", "category": "project"},
            # 配置信息
            {"q": "搜索API", "a": "Tavily", "category": "config"},
            {"q": "Tavily API key", "a": "tvly-dev", "category": "config"},
            {"q": "Helius API key", "a": "59d0dfaf", "category": "config"},
            {"q": "飞书应用权限", "a": "Wiki", "category": "config"},
            {"q": "embedding模型", "a": "bge-m3", "category": "config"},
            {"q": "LLM降级", "a": "Qwen3", "category": "config"},
            # 运营需求
            {"q": "TG bot收费", "a": "analysis", "category": "ops"},
            {"q": "bot免责声明", "a": "投资建议", "category": "ops"},
            {"q": "Kline问题", "a": "实时更新", "category": "ops"},
            {"q": "birdeye替代", "a": "免费", "category": "ops"},
            {"q": "每日快报", "a": "Solana", "category": "ops"},
            {"q": "ana试用", "a": "五次", "category": "ops"},
        ]
        ...
        for mode in ["keyword", "hybrid", "vector"]:
            ...
            for i, tq in enumerate(test_queries):
                ...
                if mode == "hybrid":
                    ...
                    payload["vector_weight"] = 0.4
                    payload["keyword_weight"] = 1.6
    ...
    async def run_all(self):
        print("="*60)
        print("Mnemonic 四维评估 v4 (Real Session Data)")
...
async def main():
    ...
    with open("eval_report_v4.json", "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"\nReport saved to eval_report_v4.json")
