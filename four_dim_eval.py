...
REAL_NS = "hermes:boss:hnoe:memory_import"
...
class FourDimEvaluator:
    ...
    async def test_d2_recency(self):
        """D2: 矛盾信息覆盖 (冲突解决率 >= 90%) - 每个case独立namespace"""
        print("\n" + "="*60)
    ...
    async def test_d4_relevance(self):
        """D4: 搜索相关性 - 使用真实记忆数据+多种搜索模式"""
        print("\n" + "="*60)
        print("D4: Relevance Test (L1>=75%, L3>=0.85)")
        print("="*60)
        
        # 使用已写入的真实记忆 (hermes:boss:hnoe:memory_import)
        # 先确认数据存在
        resp = await self.client.get(
            f"{API_BASE}/memories",
        ...
        test_queries = [
            {"q": "Boss是做什么的", "a": "Web3运营", "category": "identity"},
            {"q": "Boss的运营重点", "a": "用户增长", "category": "identity"},
            {"q": "HNOE角色定位", "a": "运营", "category": "identity"},
            {"q": "Boss对文案的要求", "a": "去AI味", "category": "preference"},
            {"q": "Boss沟通语言", "a": "中文", "category": "preference"},
            {"q": "coinAnalysis是什么", "a": "Solana Token分析", "category": "project"},
            {"q": "coinAnalysis技术栈", "a": "Python", "category": "project"},
            {"q": "zSunKoin的交易周期", "a": "15分钟", "category": "project"},
            {"q": "zSunKoin杠杆设置", "a": "3-10x", "category": "project"},
            {"q": "zSunKoin核心策略", "a": "趋势跟随", "category": "project"},
            {"q": "Mnemonic用什么数据库", "a": "PostgreSQL", "category": "project"},
            {"q": "Mnemonic的API端口", "a": "8010", "category": "project"},
            {"q": "搜索用什么API", "a": "Tavily", "category": "config"},
            {"q": "飞书缺少什么权限", "a": "Wiki", "category": "issue"},
            {"q": "embedding用的什么API", "a": "SiliconFlow", "category": "config"},
        ]
        ...
        for mode in ["hybrid", "keyword", "vector"]:
            ...
            for i, tq in enumerate(test_queries):
                payload = {"query": tq["q"], "limit": 5, "mode": mode, "min_importance": 0.3}
                if mode == "hybrid":
                    payload["vector_weight"] = 0.6
                    payload["keyword_weight"] = 1.4
                ...
                print(f"  [q-{i+1:02d}] {mark} {tq['q']} → TOP-1: {results[0]['memory']['content'][:40]}")
            ...
            l3_mrr = l3_sum / len(test_queries)
            passed = l1_score >= 0.75 and l3_mrr >= 0.85
            
            print(f"\n  {mode.upper()}: L1={l1_score:.1%}, L3(MRR)={l3_mrr:.2f} {'PASS' if passed else 'FAIL'}")
            
            self.results[f"d4_relevance_{mode}"] = {
                "scores": {"L1": round(l1_score, 3), "L3_MRR": round(l3_mrr, 3)},
                "passed": passed,
        ...
        best_mode = max(
            ...
            key=lambda m: self.results[f"d4_relevance_{m}"]["scores"]["L3_MRR"]
        )
        ...
        print(f"\n>>> D4 Best: {best_mode} | L1={best['scores']['L1']:.1%}, L3={best['scores']['L3_MRR']:.2f} {'PASS' if best['passed'] else 'FAIL'}")
    
    async def run_all(self):
        print("="*60)
        print("Mnemonic 四维评估 v3")
        ...
        for dim in ["d1_extraction", "d2_recency", "d3_robustness", "d4_relevance"]:
            ...
            if "score" in data:
                ...
                print(f"  {dim.upper()} ({mode}): L1={scores.get('L1',0):.1%}, L3={scores.get('L3_MRR',0):.2f} {status}")
            if not data["passed"]:
                all_passed = False
        ...
        for mode in ["hybrid", "keyword", "vector"]:
            ...
            print(f"    {mode}: L1={d['scores']['L1']:.1%}, L3={d['scores']['L3_MRR']:.2f}")
...
async def main():
    ...
    with open("eval_report_latest.json", "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"\nReport saved to eval_report_latest.json")
